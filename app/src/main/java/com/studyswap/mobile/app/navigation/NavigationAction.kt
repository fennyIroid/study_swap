...
sealed interface NavigationAction {
    ...
    data class NavigateWithPopUpTo(
        ...
        override fun navigate(navController: NavController, resetNavigate: (NavigationAction) -> Unit): Boolean {
            val inclusiveValue = inclusive
            val options = navOptions {
                popUpTo(popUpToRouteDefinition.value) {
    ...
    data class NavigateIntent(private val intent: Intent, private val options: Bundle? = null, private val finishCurrentActivity: Boolean = false) : NavigationActionIntent {
        override fun navigate(context: Context, resetNavigate: (NavigationAction) -> Unit): Boolean {
            context.startActivity(intent, options)
            if (finishCurrentActivity) context.requireActivity().finish()
            resetNavigate(this)
            return false
        }
    }

    data class NavigateIntentWithFinishAffinity(private val intent: Intent) : NavigationActionIntent {
        override fun navigate(context: Context, resetNavigate: (NavigationAction) -> Unit): Boolean {
    ...
    data class Pop(private val popToRouteDefinition: NavRouteDefinition? = null, private val inclusive: Boolean = false) : NavigationActionRoute {
        override fun navigate(navController: NavController, resetNavigate: (NavigationAction) -> Unit): Boolean {
            val stackPopped = if (popToRouteDefinition == null) navController.popBackStack()
            else navController.popBackStack(popToRouteDefinition.value, inclusive)
            resetNavigate(this)
            return stackPopped
        }
    }
    ...
    data class PopWithResult(
        ...
        override fun navigate(navController: NavController, resetNavigate: (NavigationAction) -> Unit): Boolean {
            val destination = if (popToRouteDefinition != null) navController.getBackStackEntry(popToRouteDefinition.value) else navController.previousBackStackEntry
